...
import { useFormContext, useFieldArray, type UseFormRegister, type FieldValues } from 'react-hook-form';
import FormContext from "~/context/FormContext";


const Ingredient = ({ index, register }: { index: number; register: UseFormRegister<FieldValues> }): JSX.Element => {
	return (
		<div className="grid grid-cols-3 gap-2 relative">
			<div className="relative mt-3 pr-4">
				<input {...register(`ingredients.${index}.amount`)} type="number" className="block rounded-t-lg pl-2.5 pb-2.5 pt-5 w-full text-sm text-gray-900 border-0 border-b border-gray-300 appearance-none dark:border-gray-400 dark:focus:border-emerald-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "/>
			...
			<div className="relative mt-3 pr-4">
				<input {...register(`ingredients.${index}.measurement`)} className="block rounded-t-lg pl-2.5 pb-2.5 pt-5 w-full text-sm text-gray-900 border-0 border-b border-gray-300 appearance-none dark:border-gray-400 dark:focus:border-emerald-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
			...
			<div className="relative mt-3 pr-4">
				<input {...register(`ingredients.${index}.ingredient`)} className="block rounded-t-lg px-2.5 pb-2.5 pt-5 w-full text-sm text-gray-900 border-0 border-b border-gray-300 appearance-none dark:border-gray-400 dark:focus:border-emerald-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
